...
export default function Notifications() {
  ...
  const styles = {
    ...
    notificationItem: { padding: '12px', borderBottom: '1px solid #E1DFDA', cursor: 'pointer', color: '#4B4E6D' },
    unreadItem: { backgroundColor: '#FADADD' },
    noNotifs: { padding: '16px', textAlign: 'center' as 'center', color: '#A8A9AD' }
  };
          ...
          {notifications.length > 0 ? (
            notifications.map(notif => (
              <div key={notif.id} onClick={() => handleNotificationClick(notif)} style={{ ...styles.notificationItem, ...(!notif.isRead ? styles.unreadItem : {}) }}>
                <strong>{notif.sender.username}</strong> posted a new murmur.
              </div>
            ))
          ) : (
            <div style={styles.noNotifs}>No notifications yet.</div>
          )}
        </div>
      )}
    </div>
